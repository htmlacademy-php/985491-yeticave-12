...
--вставка лотов
INSERT INTO lots (date_create, name, description, url_image, start_price, date_end, step_price, author_id, category_id)
...
ON lots.category_id = categories.id WHERE lots.winner_id IS NOT NULL ORDER BY date_create DESC;
...
SELECT lots.date_create, price, user_id FROM bets JOIN lots ON bets.lot_id = lots.id WHERE lots.id = 2 ORDER BY date_create DESC;
