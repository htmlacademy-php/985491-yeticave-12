  <main>
    ...
    <form class="form form--add-lot container <?php if($errors_validate): ?> form--invalid <?php endif; ?> " action="add.php" method="post" enctype="multipart/form-data">
      <h2>Добавление лота</h2>
      <div class="form__container-two">
        <div class="form__item <?php if($errors_validate['lot-name']): ?> form__item--invalid <?php endif; ?> ">
          <label for="lot-name">Наименование <sup>*</sup></label>
          <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value="<?=get_filtered_post_val('lot-name'); ?>">
        ...
        <div class="form__item <?php if($errors_validate['category']): ?> form__item--invalid <?php endif; ?>">
          ...
          <select id="category" name="category">
            ...
            <?php foreach ($categories as $category): ?>
              <option <?php if($category['id'] === get_post_val('category')): ?> selected <?php endif; ?> value="<?=$category['id'] ?>"><?=htmlspecialchars($category['name']) ?></option>
            <?php endforeach; ?>
          </select>
      ...
      <div class="form__item form__item--wide <?php if($errors_validate['message']): ?> form__item--invalid <?php endif; ?>">
        ...
        <textarea id="message" name="message" placeholder="Напишите описание лота"><?=get_filtered_post_val('message'); ?></textarea>
      ...
      <div class="form__container-three">
        <div class="form__item form__item--small <?php if($errors_validate['lot-rate']): ?> form__item--invalid <?php endif; ?>">
          ...
          <input id="lot-rate" type="text" name="lot-rate" placeholder="0" value="<?=get_filtered_post_val('lot-rate'); ?>">
        ...
        <div class="form__item form__item--small <?php if($errors_validate['lot-step']): ?> form__item--invalid <?php endif; ?>">
          ...
          <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?=get_filtered_post_val('lot-step'); ?>" >
        ...
        <div class="form__item <?php if($errors_validate['lot-date']): ?> form__item--invalid <?php endif; ?>">
          ...
          <input class="form__input-date" id="lot-date" type="text" name="lot-date" placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?=get_filtered_post_val('lot-date'); ?>" >
