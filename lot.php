...
$result_lot = mysqli_query($connection, $sql_read_lot);
...
$result_bet = mysqli_query($connection, $sql_read_bet);
...
if (isset($_POST['submit_bet'])) {  //Если есть такое поле в POST, значит форма отправлена
  ...
  if (!$errors_validate) {
    ...
    $sql_read_bet = "SELECT bets.date_create, bets.price, users.name FROM bets JOIN users ON bets.user_id = users.id WHERE bets.lot = '$id' ORDER BY bets.date_create DESC ";
    $result_bet = mysqli_query($connection, $sql_read_bet);
    $bet_open_lot = mysqli_fetch_all($result_bet, MYSQLI_ASSOC);
